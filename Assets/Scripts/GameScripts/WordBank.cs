...
public class WordBank : MonoBehaviour
{
   ...
   private void Awake()
   {
       originalWords.AddRange(strings);
       workingWords.AddRange(originalWords);
       Shuffle(workingWords);
       ConvertToLower(workingWords);
   }

   private void Shuffle(List<string> list) // shuffle the word
   {
       for(int i = 0; i < list.Count; i++) // looping through work bank
       {
           int random = Random.Range(i, list.Count);
           string temporary = list[i];

           list[i] = list[random]; // taking the random word
   ...
   private void ConvertToLower(List<string> list)
   {
       ...
       {
           list[i] = list[i].ToLower(); // converting every word into lowercase
       }
    
   }

   public string GetWord()// Getting the new word
   {
       string newWord = string.Empty;
       newWord = workingWords[Random.Range(0,workingWords.Count)];
